...
export const getAllHomestay = () => async (dispatch) => {
  dispatch(fetchProductsStart());
  try {
    const response = await axios.get('http://localhost:8080/api/v3/homestay?size=99');
    dispatch(fetchProductsSuccess(response.data.data.data)); // Lấy dữ liệu từ response.data.data
  } catch (error) {
    dispatch(fetchProductsFailure(error.message));
  }
...
export const getAllHomestayByStatus = (status) => async (dispatch) => {
  dispatch(fetchProductsStart());
  try {
    const response = await axios.get('http://localhost:8080/api/v3/homestay?size=99&statusHomestay=' +  status);
    dispatch(fetchProductsSuccess(response.data.data.data)); // Lấy dữ liệu từ response.data.data
  } catch (error) {
    dispatch(fetchProductsFailure(error.message));
  }
...
export const getAllHomestayByNameOwner = (status,name) => async (dispatch) => {
  ...
  console.log(1);
  try {
    const response = await axios.get('http://localhost:8080/api/v3/homestay?size=99&statusHomestay=' +  status + '&nameOwner=' + name);
    dispatch(fetchProductsSuccess(response.data.data.data)); // Lấy dữ liệu từ response.data.data
  } catch (error) {
    dispatch(fetchProductsFailure(error.message));
  }
...
export const getAllHomestayByHomestayName = (status,name) => async (dispatch) => {
  ...
  console.log(1);
  try {
    const response = await axios.get('http://localhost:8080/api/v3/homestay?size=99&statusHomestay=' +  status + '&nameHomestay=' + name);
    dispatch(fetchProductsSuccess(response.data.data.data)); // Lấy dữ liệu từ response.data.data
  } catch (error) {
    dispatch(fetchProductsFailure(error.message));
  }

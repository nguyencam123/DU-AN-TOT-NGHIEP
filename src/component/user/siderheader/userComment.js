import { HomeOutlined, RotateLeftOutlined, RotateRightOutlined, SwapOutlined, ZoomInOutlined, ZoomOutOutlined } from "@ant-design/icons"
    ...
    return(
        <section style={{ padding: '40px 300px 40px 300px' }}>
            ...
            <div style={{width:'100%',height:'100%',
                        ...
                        boxShadow: '0 0 3px 1px #ACAEB1',padding: '10px 10px 10px 10px'}}>
                <div style={{display:'flex'}}>
                <HomeOutlined style={{fontSize:22}}/>
                <Title level={5} style={{marginLeft:5}}>{items.homestay.name}</Title>
                <div style={{marginLeft:'auto',color:'orange',fontSize:16,display:'flex'}}>
                    <Title level={5} style={{marginLeft:5}}>Tổng tiền {formatCurrency(items.homestay.price)}</Title> (Hoàn thành)
                </div>
            ...
            <div style={{ float: 'right', marginTop: 20,marginBottom:20 }}>
                  <Pagination current={current} onChange={onChangePage} total={userComment?.totalPages * 10} />
                </div>
        </section>
    )
}

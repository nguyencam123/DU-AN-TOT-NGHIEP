...
const ForgortPasswordOwner = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            formData.append("newPass", userName);
            await message.info(
                'Đang tiến hành đặt lại mật khẩu bạn vui lòng đợi một vài giây nhé!', 2
            );
            const response = await axios.post(`http://localhost:8080/api/v2/login/reset-passwords?id=${id}`, formData);
        ...
        <section style={{ display: 'flex', justifyContent: 'center', marginTop: 100, marginBottom: 100 }}>
            <div style={{ border: '1px solid #b2b2b2', width: 500, height: '100%', borderRadius: 5, padding: 15 }}>
                ...
                <div style={{ fontWeight: '500', color: '#363535', fontSize: 16 }}>
                    Nhập mật khẩu mới của bạn</div><br />
                <div style={{ fontWeight: '500', color: '#363535', fontSize: 16 }}>
                    Mật khẩu mới</div>
                <MDBInput
                    wrapperClass='mb-4'
...
export default ForgortPasswordOwner